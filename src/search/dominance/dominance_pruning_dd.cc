...
namespace dominance {
    ...
    void DominancePruningDD::initialize(const std::shared_ptr<AbstractTask> &task) {
        DominancePruning::initialize(task);
         ...
         auto variable_ordering = VariableOrderingStrategyBasedOnStateMapping();

        std::shared_ptr<BDDManager> manager = make_shared<BDDManager>();
        vars = make_shared<SymVariables>(manager, variable_ordering, task);

/* TODO: Add option of removing spurious states
...
*/
        ...
        for (auto &local_relation: dominance_relation->get_local_relations()) {
            if (insert_dominated) {
                local_bdd_representation.push_back(
                        LocalStateRelationBDD::precompute_dominated_bdds(vars, local_relation);
            } else {
                local_bdd_representation.push_back(
                        LocalStateRelationBDD::precompute_dominating_bdds(vars, local_relation);
            }
        }
    ...
    LocalStateRelationBDD::precompute_dominating_bdds(const fts::SymbolicStateMapping &symbolic_mapping,
        ...
        for (int i = 0; i < state_relation.num_states(); ++i) {
            ...
            for (int j = 0;
                 j < state_relation.num_states(); ++j) { //TODO: Here we could iterate over the dominating states
...
*/
    ...
    bool DominancePruningBDDMap::check(const State &state, int g) {
        if (insert_dominated) {
            ...
            BDD simulatingBDD = dominance_relationSimulatingBDD(vars.get(), state);
            for (auto &entry: closed) {
                if (entry.first > g) break;
