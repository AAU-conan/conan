...
namespace operator_counting {
    void ContrastiveLandmarks::initialize_constraints(const std::shared_ptr<AbstractTask>& task, lp::LinearProgram& lp) {
    ...
    bool ContrastiveLandmarks::update_constraints_g_value(const State& state, int g_value, lp::LPSolver& lp_solver) {
        state.unpack();
        auto explicit_state = state.get_unpacked_values();

        if (std::ranges::all_of(std::views::enumerate(explicit_state), [&](auto iv){ const auto [i, v] = iv; return fts_task->get_factor(i).is_goal(v); })) {
            return false;
        }
        ...
        for (const auto& previous_state : previous_states) {
            if (previous_state.g_value <= g_value) {
                ...
                for (int i = 0; i < explicit_state.size(); ++i) {
                    std::set<int> pstate_lgs = std::views::transform(fts_task->get_factor(i).get_transitions(previous_state.state[i]), [](fts::LTSTransition tr){ return tr.label_group.group;}) | std::ranges::to<std::set<int>>();
                    auto cstate_lgs = fts_task->get_factor(i).get_transitions(explicit_state[i])
                        | std::views::transform([](fts::LTSTransition tr){ return tr.label_group.group;})
                        | std::views::filter([&](int lg){ return !pstate_lgs.contains(lg); });
                    for (const int& lg : cstate_lgs) {
                        for (const int& label : fts_task->get_factor(i).get_labels(fts::LabelGroup(lg))) {
...
: public plugins::TypedFeature<ConstraintGenerator, ContrastiveLandmarks> {
    public:
        ContrastiveLandmarksConstraintsFeature() : TypedFeature("contrastive_landmarks") {
            document_title("Contrastive Landmark constraints");
            document_synopsis("");
        }

        [[nodiscard]] std::shared_ptr<ContrastiveLandmarks> create_component(const plugins::Options &, const utils::Context &) const override {
