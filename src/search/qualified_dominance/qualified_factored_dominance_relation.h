...
/*
 * Class that represents the collection of simulation relations for a factored
 * task. Uses unique_ptr so that it owns the local relations, and it cannot be
 * copied away.
 */
...
public:
  explicit QualifiedFactoredDominanceRelation(
      std::vector<std::unique_ptr<QualifiedLocalStateRelation2>>
          &&_local_relations,
      QualifiedLabelRelation _label_relation)
      : local_relations(std::move(_local_relations)),
        label_relation(std::move(_label_relation)) {}
  ...
  const std::vector<std::unique_ptr<QualifiedLocalStateRelation2>> &
  get_local_relations() const {
  ...
  // TODO (future): Integrate methods for task transformation from the
  // fd_simulation repository
};
} // namespace qdominance
