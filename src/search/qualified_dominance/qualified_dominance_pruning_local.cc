#include "qualified_dominance_pruning_local.h"
...
namespace qdominance {
    QualifiedDominancePruningLocal::QualifiedDominancePruningLocal(bool compare_initial_state, bool compare_siblings,
                                                 const std::shared_ptr<fts::FTSTaskFactory> & fts_factory,
                                                 std::shared_ptr<QualifiedDominanceAnalysis> dominance_analysis,
                                                 utils::Verbosity verbosity) :
            QualifiedDominancePruning(fts_factory, dominance_analysis, verbosity),
            compare_initial_state(compare_initial_state),
            compare_siblings (compare_siblings) {
    }

    void QualifiedDominancePruningLocal::initialize(const std::shared_ptr<AbstractTask> &task) {
        QualifiedDominancePruning::initialize(task);
    ...
    bool QualifiedDominancePruningLocal::must_prune_operator(const OperatorProxy & op,
                                                    const State & state,
    ...
    class QualifiedDominancePruningLocalFeature
            : public plugins::TypedFeature<PruningMethod, QualifiedDominancePruningLocal> {
    public:
        QualifiedDominancePruningLocalFeature() : TypedFeature("qualified_dominance_local") {
            document_title("Local qualified dominance pruning");
        ...
        virtual shared_ptr <QualifiedDominancePruningLocal> create_component(
                const plugins::Options &opts,
                const utils::Context &) const override {

            return plugins::make_shared_from_arg_tuples<QualifiedDominancePruningLocal>(
    ...
    static plugins::FeaturePlugin<QualifiedDominancePruningLocalFeature> _plugin;
