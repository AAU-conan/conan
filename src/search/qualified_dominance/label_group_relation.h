...
namespace qdominance {
    ...
    public:
        ...
        // The pairs (lg1, lg2) where lg1 simulates lg2, and both lg1 and lg2 are relevant
        ...
        explicit LabelGroupSimulationRelation(const LabelledTransitionSystem& lts, int factor)
            : lts(lts), factor(factor) {
            const auto all_labels_groups = std::views::iota(0, lts.get_num_label_groups()) | std::views::transform(
                [](auto lg) { return LabelGroup(lg); }) | std::ranges::to<std::unordered_set>();

            std::unordered_map<int, std::set<int>> lg_sources;
            for (const auto& lg : lts.get_relevant_label_groups()) {
                lg_sources.emplace(lg.group, lts.get_transitions_label_group(lg) | std::views::transform([](const auto& tr) {
                    return tr.src;
            ...
            for (const auto& lg1 : lts.get_relevant_label_groups()) {
                for (const auto& lg2 : lts.get_relevant_label_groups()) {
                    if (std::ranges::includes(lg_sources.at(lg2.group), lg_sources.at(lg1.group))) {
            ...
            label_group_state_targets.resize(lts.get_num_label_groups(), std::vector<std::vector<int>>(lts.size()));
            for (const auto tr : lts.get_transitions()) {
                label_group_state_targets.at(tr.label_group.group).at(tr.src).push_back(tr.target);
            }
        }

        [[nodiscard]] bool simulates(const LabelGroup lg1, const LabelGroup lg2) const {
        ...
        [[nodiscard]] const std::vector<int>& targets_for_label_group_state(LabelGroup lg, int s) const {
            return label_group_state_targets.at(lg.group).at(s);
        }

        [[nodiscard]] bool compute_simulates(LabelGroup lg1, LabelGroup lg2, const QualifiedLocalStateRelation2& sim) const;
        bool compute_simulates_noop(LabelGroup lg, const QualifiedLocalStateRelation2& sim) const;
        [[nodiscard]] bool compute_noop_simulates(LabelGroup lg, const QualifiedLocalStateRelation2& sim) const;
